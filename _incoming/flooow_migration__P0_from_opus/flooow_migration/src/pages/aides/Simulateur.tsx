/**
 * Page Simulateur d'aides (/aides/simulateur)
 *
...
// Prix d'activité par défaut pour la simulation
...
const Simulateur = () => {
  ...
  const handleCalculate = async () => {
    ...
    if (result) {
      const trancheLabel = getQFBracketLabel(qf);
      ...
      setCalculationResult({
        ...
        prixInitial: prix,
        montantAPayer: result.remainingEuros,
  ...
  return (
    <PageLayout>
      <div className="container max-w-2xl px-4 py-6 space-y-6 pb-24">
        ...
        {/* Section barème (accordion) - NOUVELLES TRANCHES */}
        <Accordion type="single" collapsible className="w-full">
          <AccordionItem value="bareme" className="border rounded-lg px-4">
            ...
            <AccordionContent className="space-y-4 pt-2">
              ...
              {/* Tableau barème - NOUVELLES TRANCHES */}
              <div className="space-y-2">
                {AID_DISPLAY_BRACKETS.map((bracket, index) => {
                  ...
                  return (
                    <div key={index} className={`flex justify-between items-center p-3 rounded-lg ${colorClasses[index]}`}>
                      ...
                      <span className="font-bold">{bracket.description}</span>
                    </div>
                  );
                })}
              </div>
              ...
              <p className="text-xs text-muted-foreground italic mt-3">
                Le montant de l'aide varie selon le prix de l'activité. Barème indicatif.
              </p>
            </AccordionContent>
          </AccordionItem>
        </Accordion>
