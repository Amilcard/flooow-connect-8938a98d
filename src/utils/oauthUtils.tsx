/**
 * Utilitaires pour l'authentification OAuth
 *
 * Centralise les appels aux providers OAuth via Supabase Auth
 */
...
/**
 * Déclenche l'authentification OAuth pour un provider donné
 * @param provider - Le provider OAuth (google, apple, facebook, linkedin_oidc, azure)
 * @returns Promise avec les données de redirection ou erreur
 */
export async function signInWithProvider(provider: OAuthProvider) {
  const { data, error } = await supabase.auth.signInWithOAuth({
    provider,
    options: {
      redirectTo: authConfig.getRedirectUrl(),
      // Pour récupérer les infos de profil
      queryParams: {
        access_type: 'offline',
        prompt: 'consent',
      },
...
/**
 * Récupère la liste des providers OAuth activés, triés par priorité
 */
export function getEnabledProviders() {
  return authConfig.OAUTH_PROVIDERS
    .filter(p => p.enabled)
    .sort((a, b) => a.priority - b.priority);
}

/**
 * Icônes SVG pour chaque provider OAuth
 */
export const providerIcons: Record<OAuthProvider, JSX.Element> = {
  google: (
