...
const mapActivityFromDB = (dbActivity: any): Activity => {
  // Build images array: prioritize images array, then image_url, then empty (fallback kicks in)
  const imagesFromDB = dbActivity.images && Array.isArray(dbActivity.images) && dbActivity.images.length > 0
    ? dbActivity.images
    : dbActivity.image_url
      ? [dbActivity.image_url]
      : [];
